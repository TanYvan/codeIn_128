...
class PubTool
  ...
  def self.get_dear_money(code)
    ...
    if PubTool.new.get_first_record(@account2)!=nil
      @d_m=""
      @account2.each do |ac|
        @d_m = @d_m + get_sign(ac.currency) + SysArg.compart1(ac.f_money.to_d) + "  "
      end
      if @d_m!=""
  ...
  def self.get_indefinite_money(code)
    ...
    if PubTool.new.get_first_record(@account4)!=nil
      @d_m2=""
      @account4.each do |ac|
        @d_m2 = @d_m2 + get_sign(ac.currency)+ SysArg.compart1(ac.f_money.to_d) + "  "
      end
      if @d_m2!=""
