...
class WindUpCaseController < ApplicationController
    ...
    #删除撤销及不予执行的记录
    def backout_and_unrun_delete
        @tb_case = TbCase.find_by_recevice_code(session[:recevice_code_3])  
        @tb_case.yarddecide=nil        
        if  @tb_case.save!
            flash[:notice] = "删除成功！"
            redirect_to :action =>"backout_and_unrun",:recevice_code =>session[:recevice_code_3]
        else
    ...
    def upload_file
        ...
        if !file.original_filename.empty?  
            ...
            if File.exists?("#{SysArg.find_by_code("8010").val}/upload/backoutandunrun/#{session[:recevice_code_3]}/")
                File.open("#{SysArg.find_by_code("8010").val}/upload/backoutandunrun/#{session[:recevice_code_3]}/#{f_name}","wb") do |f|  
                    f.write(file.read) 
                end
            else 
                Dir.mkdir("#{SysArg.find_by_code("8010").val}/upload/backoutandunrun/#{session[:recevice_code_3]}")
                File.open("#{SysArg.find_by_code("8010").val}/upload/backoutandunrun/#{session[:recevice_code_3]}/#{f_name}","wb") do |f|
                    f.write(file.read) 
                end
            end
...
#        send_file("#{SysArg.find_by_code("8010").val}/upload/backoutandunrun/#{session[:recevice_code_3]}/" + utf8_to_gbk.iconv(@tb_backoutfile.filename)) unless @tb_backoutfile.filename.blank?
#       
#    end
#    # 删除裁决文件
#    def delete_file
...
#    end
    ...
    #删除执行情况的记录
    def show_implement_delete
        @tb_case = TbCase.find_by_recevice_code(session[:recevice_code_3])  
        @tb_case.runstyle=nil        
        if  @tb_case.save!
            flash[:notice] = "删除成功！"
            redirect_to :action =>"show_implement",:recevice_code =>session[:recevice_code_3]
        else
            flash[:notice] = "删除失败！"            
        end       
    end
  ...
  def book_create
    ...
    @book.recevice_code=session[:recevice_code_3]
    @book.case_code=session[:case_code_3]
    @book.general_code=session[:general_code_3]
    @book.typ="0003"
  ...
  def book_upload_do
    ...
    if file_arry.size > 1
      @doc.book_name="#{@doc.typ}_#{@doc.recevice_code}_#{@case_1.id}_#{@case_1.book_num}." + file.original_filename.split(".")[file_arry.size - 1]
    else
      @doc.book_name="#{@doc.typ}_#{@doc.recevice_code}_#{@case_1.id}_#{@case_1.book_num}"
    end
    #设置文档保存的路径，自由设置
