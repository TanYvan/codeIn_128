class PlaceRecallController < ApplicationController
  def case_list  
    ...
    c="(tb_cases_state>=4 and tb_cases_state<=6)"
    p=PubTool.new()
  ...
  def recall_do
    ...
    if @case.state==4
      @case.state=1
      TbCaseFlow.del_flow(@case.recevice_code,'0004')
      if @case.update_attributes(params[:case]) 
        flash[:notice]="咨询流水号:#{@case.recevice_code}。立案招回成功"
      else
        flash[:notice]="咨询流水号:#{@case.recevice_code}。立案招回失败"
      end
    end
