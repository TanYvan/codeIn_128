...
#    @total_m=TbCase.sum(:amount,:conditions=>["used='Y' and clerk=? and state>=4 and state<100  and end_code=''",params[:user_code]])
  ...
  def case_now_clerk
    ...
    c="tb_cases_clerk='#{session[:user_code]}' and tb_cases_state>=4 and tb_cases_state<100 and tb_cases_state<>2 and tb_cases_state<>3 and tb_cases_caseendbook_id_first is null"
    p=PubTool.new()
  ...
  def list_1
    ...
    @hant_search_1_list=['tb_cases_case_code',
      ...
      'tb_cases_nowdate','IFNULL(tb_cases_amount,0)','IFNULL(tb_contractsigns_contractdate,\\\'\\\')']
    @order=params[:order]
    if @order==nil or @order==""
...
#    end
  ...
  def list_31
    ...
    @hant_search_1_list=['tb_cases_recevice_code','tb_cases_general_code','tb_cases_case_code',
      'tb_cases_end_code','tb_cases_terms_drafting_party','IFNULL(tb_parties_partyname,\\\'\\\')','IFNULL(tb_cases_arbitprot,\\\'\\\')',
      'tb_cases_nowdate','IFNULL(tb_cases_amount,0)','IFNULL(tb_contractsigns_contractdate,\\\'\\\')','IFNULL(tb_casedelays_requestdate,\\\'\\\')']
    @order=params[:order]
    if @order==nil or @order==""
      @order="right(tb_cases_case_code,7) desc"
    end
  ...
  def list_38
    ...
    @case_pages,@case=paginate(:v_case_query1_list8s,:order=>@order,:conditions=>c,:per_page=>@page_lines.to_i)
    @c_l=VCaseQuery1List8.find_by_sql("select tb_cases_recevice_code from v_case_query1_list8s where #{c}")
  end
  ...
  def list_101
    ...
    @state={-1=>"重审",1=>"咨询",2=>"历史",3=>"不受理",4=>"立案",5=>"组庭",6=>"开庭",100=>"结案",150=>"归档申请",200=>"存档接收",220=>"归档"}
  ...
  def case_now_total
    ...
    c = "(tb_cases.state=1 or  (tb_cases.state>=4 and tb_cases.state<100)) and tb_cases.clerk_2=users.code and tb_cases.end_code='' and  tb_cases.used='Y' and users.used='Y' and  tb_cases.receivedate>='#{@date1}' and tb_cases.receivedate<='#{@date2}'"
  ...
  def list_everyone
    ...
    c ="(tb_cases.state=1 or  (tb_cases.state>=4 and tb_cases.state<100))  and tb_cases.clerk_2=? and  tb_cases.end_code='' and tb_cases.used='Y' and tb_cases.receivedate>=? and tb_cases.receivedate<=?"
  ...
  def expert_meeting_query
    ...
    @hant_search_1=[
      ...
      ['number','c.state','案件状态','select',[[1,"咨询"],[2,"历史"],[3,"不受理"],[4,"立案"],[5,"组庭"],[6,"开庭"],[-1,"重审"],[100,"结案"],[150,"归档申请"],[200,"已归档"]]],
 ...
 as e on c.used='Y' and e.used='Y' and c.recevice_code=e.recevice_code   where #{c} group by c.recevice_code order by ?",@order],@page_lines.to_i)
  ...
  def expert_meeting_list
    @page_lines=params[:page_lines]
      if @page_lines==nil or @page_lines==""
        @page_lines=@page_lines=session[:lines]
      end
      c="recevice_code=? and used='Y'"
      @expert_meetings =ExpertMeeting.find(:all,:conditions =>[c,params[:recevice_code]])
  end
end
