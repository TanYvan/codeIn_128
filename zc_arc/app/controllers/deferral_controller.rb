...
class DeferralController < ApplicationController
    def p_set
      if params[:p_typ]=='0001'
        typ2=TbParty.find(:all,:conditions=>"recevice_code='#{session[:recevice_code_4]}' and partytype=1 and used='Y'",:order=>'id',:select=>"id as id ,partyname as name")
      elsif params[:p_typ]=='0002'
        typ2=TbParty.find(:all,:conditions=>"recevice_code='#{session[:recevice_code_4]}' and partytype=2 and used='Y'",:order=>'id',:select=>"id as id,partyname as name")
      elsif params[:p_typ]=='0003'
        typ2=TbAgent.find(:all,:conditions=>"recevice_code='#{session[:recevice_code_4]}' and partytype=1 and used='Y'",:order=>'id',:select=>"id,name")
      elsif params[:p_typ]=='0004'
        typ2=TbAgent.find(:all,:conditions=>"recevice_code='#{session[:recevice_code_4]}' and partytype=2 and used='Y'",:order=>'id',:select=>"id,name")
      end
    ...
    def deferral_create
       ...
       @deferral.recevice_code=session[:recevice_code_4]
       @deferral.case_code=session[:case_code_4]
       @deferral.general_code=session[:general_code_4]
