...
class ArbitroomController < ApplicationController
  ...
  def arbitroom_list
      ...
      @hant_search_1=[
        ...
        ['char','sittingplace','仲裁庭','select',TbDictionary.find(:all,:conditions=>"typ_code='0023' and state='Y'",:order=>'data_val',:select=>"data_val,data_text").collect{|p|[p.data_val,p.data_text]}],
        ['char','open_t','开庭时间','text',TbDictionary.find(:all,:conditions=>"typ_code='0024' and state='Y'",:order=>'data_val',:select=>"data_text").collect{|p|[p.data_text,p.data_text]}],
        ['char','close_t','闭庭时间','text',TbDictionary.find(:all,:conditions=>"typ_code='0024' and state='Y'",:order=>'data_val',:select=>"data_text").collect{|p|[p.data_text,p.data_text]}],
