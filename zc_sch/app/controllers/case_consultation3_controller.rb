...
tb_cases_end_code,tb_cases_clerk from v_case_query1_list1s where tb_parties_partyname like '%#{partyname}%'")
  ...
  def clerk_select_list
    if session["0001"]=="Y"
      ...
      @clerks=User.find_by_sql("select code ,name from users where  users.used='Y' and  users.code in (select user_code from user_duties where duty_code='0003' and user_code='#{session[:user_code_2]}')")
    else
