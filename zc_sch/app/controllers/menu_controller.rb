class MenuController < ApplicationController
  ...
  def tree
    ...
    if params[:menu_id]=="0"
      m=Menu.find_by_sql("select distinct menus.role_code as role_code,menus.code as code,menus.name as name,menus.url as url,menus.parent as parent from menus,urs where urs.user_code='#{session[:user_code]}' and menus.role_code=urs.role_code and menus.parent='0'  order by role_code,menus.code") 
    else
      m=Menu.find_by_sql("select distinct menus.role_code as role_code,menus.code as code,menus.name as name,menus.url as url,menus.parent as parent from menus,urs where urs.user_code='#{session[:user_code]}' and menus.role_code=urs.role_code and menus.id='#{params[:menu_id]}'  order by role_code,menus.code") 
    end
    if m.empty?
       return "" 
     else
