class DocApprovalController < ApplicationController
  def list
    @state={-1=>"待重新审批",0=>"待审批",1=>"批准",2=>"不批准",3=>"需要修改"}
    @hant_search_1_page_name="list"
    @hant_search_1=[['number','state','状态','select',[['','全部'],[0,'待审批'],[1,'批准'],[2,'不批准'],[3,'需要修改'],[-1,'待重新审批']]],['char','recevice_code','咨询流水号','text',[]],['char','send_serial_code','发文流水号','text',[]],['char','send_code','发文编号','text',[]]]
    @order=params[:order]
    ...
    c="a_u='#{session[:user_code]}' and state<>0 and  state<>-1 "
    p=PubTool.new()
