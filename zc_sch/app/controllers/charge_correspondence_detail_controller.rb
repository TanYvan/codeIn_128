class ChargeCorrespondenceDetailController < ApplicationController
  ...
  def charge_case_list
    ...
    if @order==nil or @order==""
      @order="tb_cases.recevice_code desc"
    end
    ...
    c="tb_cases.used='Y' and tb_charges.used='Y' and tb_cases.recevice_code=tb_charges.recevice_code and ((tb_cases.clerk='#{session[:user_code]}' and tb_cases.state>=4 and tb_cases.state<=100) or ((tb_cases.clerk='#{session[:user_code]}' or tb_cases.clerk_2='#{session[:user_code]}') and tb_cases.state>=1 and tb_cases.state<3)) "
    p=PubTool.new()
    ...
    @charge_pages,@charge=paginate_by_sql(TbCharge,"select distinct tb_charges.recevice_code as recevice_code from tb_charges,tb_cases where #{c}  order by #{@order}",@page_lines.to_i)
  end
