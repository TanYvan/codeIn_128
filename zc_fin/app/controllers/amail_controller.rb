...
class AmailController < ApplicationController
  ...
  def list
    ...
    if session[:recevice_code_4]==nil
      ...
      @amail=TbAmail.find(:all,:conditions=>c,:order=>"recevicedate desc")
    end
  end
  ...
  def create
    ...
    if params[:condi]!=''
      ...
      if @s1=='a'
        @amail.submitman='0001' #申请人
      elsif @s1=='b'
        @amail.submitman='0002' #被申请人
      else
        @amail.submitman=nil
      end
      @amail.arbitman = params[:condi].slice(1, params[:condi].length-1)
    end
    puts "--------------------"
    puts @amail.arbitman
    puts "--------------------"
