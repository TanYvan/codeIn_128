class Remuneration8SetController < ApplicationController
  
  def rmb_set 
    render :update do |page|
      cov  = Iconv.new('utf-8','gbk')  # Iconv 编码转换
      page.remove 'remun_rmb'
      page.replace_html 'rmb_set', :partial => 'rmb',:object=>TbDictionary.find(:first,:conditions=>"typ_code='0053' and data_text='#{cov.iconv(params[:remun_grade])}'").data_val
    end
  end
  ...
  def rmb_all_set 
    render :update do |page|
      cov  = Iconv.new('utf-8','gbk')  # Iconv 编码转换
      val=TbDictionary.find(:first,:conditions=>"typ_code='0053' and data_text='#{cov.iconv(params[:grade])}'").data_val
      i=params[:i]
      c={"i"=>i,"val"=>val}
      ...
      if TbBonusPenalty.find(:first,:conditions=>"recevice_code='#{session[:recevice_code_2]}' and typ='#{r.p_typ}' and p='#{r.p}' and used='Y' and extend_code=''")!=nil 
        ...
        if params["hid_name_"+r.id.to_s]!=nil
          ...
          if @r_u.rmb==params["old_rmb_name_"+r.id.to_s].to_f
            ...
            @r_u.t2=Time.now()
            if @r_u.save
              TbBonusPenalty.up_set(session[:recevice_code_2],@r_u.p_typ,@r_u.p,'zc_rmb',params["old_rmb_name_"+r.id.to_s].to_f,@r_u.rmb)
            end
          end
        end
      end
