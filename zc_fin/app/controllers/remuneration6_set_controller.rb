class Remuneration6SetController < ApplicationController
  def rmb_set 
    render :update do |page|
      ...
      page.replace_html 'rmb_set', :partial => 'rmb',:object=>TbDictionary.find(:first,:conditions=>"typ_code='0052' and data_text='#{cov.iconv(params[:remun_grade])}'").data_val
    end
  end
  ...
  def rmb_all_set 
    render :update do |page|
      cov  = Iconv.new('utf-8','gbk')  # Iconv 编码转换
      val=TbDictionary.find(:first,:conditions=>"typ_code='0052' and data_text='#{cov.iconv(params[:grade])}'").data_val
      i=params[:i]
      c={"i"=>i,"val"=>val}
