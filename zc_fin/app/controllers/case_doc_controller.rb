...
#        end
  ...
  def get_con_other(recevice_code)
    #结案感谢函类别分辨
    @con_other=""#其它条件
    @caseorg = TbCaseorg.find_by_recevice_code_and_used(params[:recevice_code])

    if @case.special_convention=="0001"
       ...
       if arbit_record.index("简易") || arbit_record.index("1人")
         @con_other="and code not in ('0103')"
       else
         @con_other="and code not in ('0102')"
       end
