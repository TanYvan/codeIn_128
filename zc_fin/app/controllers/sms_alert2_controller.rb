class SmsAlert2Controller < ApplicationController
  ...
  def list
    ...
    if recevice_code==nil
      ww="#{session[:user_code]}-sitting-%"
    else
      ww="%-sitting-#{recevice_code}-%"
    end
    c="used='Y' and typ='0002' and  send_state<900 and (content_id like  '#{ww}')"
    p=PubTool.new()
  ...
  def delete
    ...
    if @sms.save
      ...
      flash[:notice]="删除失败"
      render :action=>"list",:search_condit=>params[:search_condit],:recevice_code=>params[:recevice_code],:order=>params[:order],:page=>params[:page],:page_lines=>params[:page_lines]
    end
  end
  ...
  def list_1
    ...
    c="used='Y'  and typ='0002'  and  send_state>=900  and (content_id like '#{session[:user_code]}-sitting-%' )"
    p=PubTool.new()
    if @search_condit!= " and " and p.sql_check_3(@search_condit)!=false
      c = c + @search_condit
    end
