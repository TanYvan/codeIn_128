...
class ShowExtendController < ApplicationController
  ...
  def invoice
    @extend_code=TbTExtendCode.find_by_t_extend_code(params[:t_extend_code])
    @extend = TbExtend.find(:all, :conditions => ["used='Y' and t_extend_code=?", params[:t_extend_code]],:select =>" should_rmb, t_extend_code,p_typ,p,p_name,id_card,tax_base_rmb ",:order=>"id")
  ...
  def invoice_h
    @extend_code=TbTExtendCode.find_by_t_extend_code(params[:t_extend_code])
    @extend = TbExtend.find(:all, :conditions => ["used='Y' and typ<>'5' and t_extend_code=?", params[:t_extend_code]],:select =>" should_rmb, t_extend_code,p_typ,p,p_name,id_card,tax_base_rmb ",:order=>"id")
    send_data(render,:type => "application/msword",:filename=> Iconv.iconv('gbk','utf-8',"代开发票清单.doc").to_s, :disposition => 'attachment')
  end
