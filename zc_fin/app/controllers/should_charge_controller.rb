class ShouldChargeController < ApplicationController
  ...
  def create
    ...
    if @should.save
      if @should.typ == "0004"
        # 华南仲裁委个性化。无明确争议金额收费的划分方法：
        # 1）涉外案件，无明确争议金额收费作为仲裁费收取，不用再划分；
        # 国内案件，对于存在了明确或者不明确争议金额的，根据已经算出的受理费、处理费之间的比例，划分无明确争议金额收费的受理费、处理费；
        # 不存在明确或者不明确争议金额的，不用划分，受理费为零、处理费为零。
        if @case.aribitprog_num != '0001' and @case.aribitprog_num != '0002' #国内普通，国内简易
          ...
          @should_new_2.save
          # 如果既没有收取过立案费，即本请求或者反请求都没有提出争议金额，那么应该允许手工录入立案费
          if TbShouldCharge.find(:all, :conditions => "used='Y' and recevice_code='#{params[:recevice_code]}' and typ = '0001'").empty?
            shc = TbShouldCharge.new()
            shc.recevice_code = params[:recevice_code]
            shc.case_code     = @case.case_code
            shc.general_code  = @case.general_code
            shc.u             = @should_new_2.u
            shc.u_t           = Time.now()
            shc.typ           = "0005"
            shc.payment       = @should.payment
            shc.rmb_money     = 0
            shc.currency      = @should.currency
            shc.f_money       = 0
            shc.re_rmb_money  = 0
            shc.rate          = @should.rate
            shc.parent_id     = @should.id
            shc.save
          end
        else
          if TbShouldCharge.find(:all, :conditions => "used='Y' and recevice_code='#{params[:recevice_code]}' and typ = '0001'").empty?
  ...
  def delete
    ...
    if TbChargeCorr.find(:all,:conditions=>"used='Y' and state<>3 and should_charge_id='#{@should.id}'").empty?
      @should.used="N"
      if session[:user_code]!=nil
        @should.u=session[:user_code]
      else
