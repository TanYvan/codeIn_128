class ArbitmanSelectController < ApplicationController
  def p_set
    ...
    if PubTool.new().sql_check_3(spell)!=false
      @tb_arbitmen =TbArbitman.find_by_sql("select * from tb_arbitmen where used='Y' and ((tb_arbitmen.spell like '#{spell}%') or (tb_arbitmen.name like  '#{spell}%'))   order by tb_arbitmen.spell,tb_arbitmen.name")
