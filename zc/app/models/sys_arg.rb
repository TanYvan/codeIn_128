class SysArg < ActiveRecord::Base
  ...
  def self.round_2(v)
    r = v * 100
    rrr = r.to_s.to_f.round
    return rrr.to_f / 100
  end
  def self.round_3(v)
    r = v * 1000
    rrr = r.to_s.to_f.round
    return rrr.to_f / 1000
  end
  #金额数字分割################### 如果为0则返回下划线
  ...
  def self.add_0005_1
    r=""
    s=self.find(:first,:conditions=>"code='0005_1'")
    r=sprintf("%09d",s.val.to_i + 1)
    s.val=r
    s.save
    return r
  end
  ...
  def self.add_0006_1
    r=""
    s=self.find(:first,:conditions=>"code='0006_1'")
    old_ym=s.val.slice(0,6)
    old_c=s.val.slice(6,3)
    n=Date.today.to_s.slice(0,4)+Date.today.to_s.slice(5,2)
    if n>old_ym
  ...
  #员工报酬发放编号下降
  def self.reduce_0006_1
    r=""
    s=self.find_by_code("0006_1")
    extend_code=TbSelfTExtendCode.find(:first,:conditions=>"used='Y'",:order=>"t_extend_code desc")
    if extend_code
      s.val=extend_code.t_extend_code
    else
      s.val="199710001"
  ...
  def self.add_0009_2
    r=""
    s=self.find(:first,:conditions=>"code='0009_2'")
    old_year=s.val.slice(0,4)
    old_c=s.val.slice(4,6)
    n=Date.today.to_s.slice(0,4)
    if n>old_year
  ...
  def self.add_2011
    r=""
    s=self.find(:first,:conditions=>"code='2011'")
    r=(s.val.to_i + 1).to_s
    s.val=r
    s.save
    return "party#{r}"
  ...
  def self.get_stime(time1)
    open_time={"08:00"=>"上午八时","08:30"=>"上午八时三十分","09:00"=>"上午九时","09:30"=>"上午九时三十分","10:00"=>"上午十时","10:30"=>"上午十时三十分","11:00"=>"上午十一时","11:30"=>"上午十一时三十分","12:00"=>"中午十二时",
      "12:30"=>"十二时三十分","13:00"=>"十三时","13:30"=>"十三时三十分","14:00"=>"十四时","14:30"=>"十四时三十分","15:00"=>"十五时","15:30"=>"十五时三十分",
      "16:00"=>"十六时","16:30"=>"十六时三十分","17:00"=>"十七时","17:30"=>"十七时三十分","18:00"=>"十八时","18:30"=>"十八时三十分",
      "19:00"=>"十九时","19:30"=>"十九时三十分","20:00"=>"二十时"}
