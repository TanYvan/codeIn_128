class Cost
  ...
  def charge_0006_01(typ1,applicant_all,respondent_all)
      if typ1 == '1'
        #puts applicant_all
        if applicant_all != 0
          registration_fee = Cost.new.get_0006_01_01(applicant_all)
          arbitration_fee  = Cost.new.get_0006_01_02(applicant_all)
        else
          registration_fee = 0
          arbitration_fee  = 0
        end
        ...
        if respondent_all != 0
          registration_fee = Cost.new.get_0006_01_01(respondent_all)
          arbitration_fee  = Cost.new.get_0006_01_02(respondent_all)
        else
          registration_fee = 0
          arbitration_fee  = 0
        end
      end
      return {"arbitration_fee" => arbitration_fee,"registration_fee" => registration_fee}
  end

  #SCIA规则-国内案件受理费
  def get_0006_01_01(ccc)
    if ccc<0
      rrr=0
  ...
  #SCIA规则-国内案件处理费
  def get_0006_01_02(ccc)
    if ccc<0
      rrr=0
    elsif ccc>=0  && ccc<200000
      rrr=5000
    elsif ccc>=200000 && ccc<500000
  ...
  # applicant_all:申请方争议金额
  # respondent_all:被申请方争议金额
  def charge_0006_02(typ1,applicant_all,respondent_all)
      if typ1 == '1'
        #puts applicant_all
        if applicant_all != 0
          registration_fee = Cost.new.get_0006_02_01(applicant_all)
          arbitration_fee  = Cost.new.get_0006_02_02(applicant_all)
        else
          registration_fee = 0
          arbitration_fee  = 0
        end
        ...
        if respondent_all != 0
          registration_fee = 0#Cost.new.get_0006_02_01(respondent_all)
          arbitration_fee  = Cost.new.get_0006_02_02(respondent_all)
        else
          registration_fee = 0
          arbitration_fee  = 0
        end
  ...
  #SCIA规则-涉外案件立案费
  def get_0006_02_01(ccc)
    rrr=10000
    return PubTool.new.round_0(rrr)
  end
  #SCIA规则-涉外案件仲裁费
  def get_0006_02_02(ccc)
    if ccc<0
      rrr=0
  ...
  # applicant_all:申请方争议金额
  # respondent_all:被申请方争议金额
  def charge_01(typ1,applicant_all,respondent_all)
      if typ1 == '1'
        #puts applicant_all
        if applicant_all != 0
  ...
  # applicant_all:申请方争议金额
  # respondent_all:被申请方争议金额
  def charge_02(typ1,applicant_all,respondent_all)
     if typ1=='1'
        if applicant_all != 0
  ...
  # applicant_all:申请方争议金额
  # respondent_all:被申请方争议金额
  def charge_03(typ1,applicant_all,respondent_all)
     if typ1 == '1'
        if applicant_all != 0
  ...
  def get_fee(typ1,typ2,recevice_code)
    ...
    if @case != nil
      #在配置表中查找费用算法函数
      @cfd=ChargeFunDetail.find_by_role_code_and_app_code_and_used(@case.app_rules, @case.aribitprog_num,"Y")
      rrr=Cost.new.method(@cfd.fun_code).call(typ1, applicant_all, respondent_all)
      registration_fee = rrr["registration_fee"]
      arbitration_fee = rrr["arbitration_fee"]
      if typ2 == 1
