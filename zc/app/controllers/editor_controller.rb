class EditorController < ApplicationController
  ...
  def upload_editor_image  
    ...
    File.open("#{File.expand_path(RAILS_ROOT)}/public/editor/editor_image/#{filename}", "wb") do |f|      
      f.write(file.read)           
    end       
    render :text => "<script>window.parent.LoadIMG('/editor/editor_image/#{filename}')</script>"  
  rescue  
  ...
  def upload_editor_attach   
    ...
    filename = Time.now.strftime("%Y%m%d%H%M%S") + rand(10000).to_s + "." + filename[0]     
    File.open("#{File.expand_path(RAILS_ROOT)}/public/editor/editor_attach/#{filename}", "wb") do |f|      
      f.write(file.read)           
    end         
    render :text => "<script>window.parent.LoadAttach('/editor/editor_attach/#{filename}')</script>"  
  rescue  
