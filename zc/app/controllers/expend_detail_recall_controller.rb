class ExpendDetailRecallController < ApplicationController
  ...
  def list
    ...
    c="used='Y' and (state=2 or state=3 or state=4)"
    p=PubTool.new()
    if @search_condit!= " and " and p.sql_check_3(@search_condit)!=false
      c = c + @search_condit
    end

    @should_pages,@should=paginate(:tb_expend_details,:order=>@order,:conditions=>c,:per_page=>@page_lines.to_i)
  end
  ...
  def detail_list
    ...
    @should=TbExpendDetail.find(:all,:order=>'id',:conditions=>c)
  end
