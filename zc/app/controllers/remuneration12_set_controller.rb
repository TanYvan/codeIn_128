class Remuneration12SetController < ApplicationController
  ...
  def list
    ...
    if session[:recevice_code_2]!=nil
      ...
      if ca_rc == nil or @xs.blank?
        ...
        @tzxs = @xs.xishu2 # 仲裁庭报酬调整系数
        @yyy  = SysArg.round_2(@zcf3 * @tzxs)#.round
        @jjj   = SysArg.round_2(@yyy * 0.7)#.round

        @evaluate = "yes"  # 是否计算稿酬和绩效报酬标记，yes：计算；no：不计算。如果是独任仲裁员案件，且仲裁费（税后）小于 20000，不需要计算
  #      if typ == 1 and @zcf2 <= 20000
  #        @evaluate = "no"
  #      end
        if TbCasedelay.find_by_recevice_code_and_used(@case.recevice_code,"Y") == nil && @case.finally_limit_dat && @case.caseendbook_id_last!=0 && (@case.finally_limit_dat - TbCaseendbook.find(@case.caseendbook_id_last).decideDate) > 30
          @point = "本案没有延期,而且提前#{@case.finally_limit_dat - TbCaseendbook.find(@case.caseendbook_id_last).decideDate}天结案"
        end
      end
    end
  end
  ...
  def update_1
    ...
    if remun.used == 'Y' and bp != nil and remun.rmb == params[:old_rmb].to_i and  !query_state(session[:recevice_code_2])# 如果该仲裁员的报酬还未发放且未提交才可以修改
      if evaluate == "yes" # 如果是独任仲裁员案件，且仲裁费（税后）小于 20000，则只评价，不计算
       remun.rmb = (kkk * params[:remun][:grade_xs].to_f / 100).round #
      ...
      if remun.update_attributes(params[:remun])
        TbBonusPenalty.up_set(session[:recevice_code_2],'0001',remun.p_code,'gc_rmb', old_rmb,remun.rmb)
        flash[:notice]="操作成功"
    ...
    begin
      ...
      casearbitman.each do |man|
        ...
        if man.arbitmantype == "0001" or man.arbitmantype == "0002" # 首席或独任
          rem.rmb     = (jjj * 0.4).round # 首席或独任仲裁员报酬为 jjj（仲裁员基本报酬）的50%
        else
          rem.rmb     = (jjj * 0.3).round # 普通仲裁员报酬为 jjj（仲裁员基本报酬）的25%
        end
...
#        end
  ...
  def changexs
    ...
    xs.xishu2 = params[:xs][:xishu2]
    xs.remark = params[:xs][:remark]
...
#      end
  ...
  def state_change
    ...
    if @case.remun_state == 'N'
      @case.remun_state = 'Y'
      @case.remun_dt=DateTime.now.to_s
    else
      @case.remun_state = 'N'
      @case.remun_dt=nil
    end
