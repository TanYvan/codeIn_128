class CaseConsultationController < ApplicationController
    ...
    if partyname.strip != '' and PubTool.new().sql_check_3(partyname) != false
      p_case = VParty.find_by_sql(
        " select distinct partyname,recevice_code,case_code,nowdate,end_code,caseendbook_id_last,clerk,mobiletel,area,tel
          from  v_parties
          where partyname like \"%#{cov.iconv(partyname)}%\" order by tel desc"
      )
    else
      p_case = nil
    end
  ...
  def list
    ...
    @hant_search_1=[['char',' recevice_code) in (select recevice_code  from  tb_parties where partyname','当事人名称(不能联合查询)','text',[]],['char','recevice_code','咨询流水号','text',[]],['date','receivedate','咨询日期','text',[]],['number','amount','争议金额','text',[]]]
    @order=params[:order]
...
#    session[:general_code_4]=params[:general_code]
  ...
  def clerk_select_list
    if session["0001"]=="Y"
      ...
      @clerks=User.find_by_sql("select code ,name from users where  users.used='Y' and users.states='Y' and  users.code in (select user_code from user_duties where duty_code='0003' and user_code='#{session[:user_code]}')")
    else
  ...
  def set_case2
     ...
     if @case.save
        SysArg.get_pn_by_recevice_code(@case.recevice_code)
        flash[:notice]="#{params[:recevice_code]}案件成功设为历史案件"
     else
        flash[:notice]="#{params[:recevice_code]}案件设置历史案件失败"
     end
     redirect_to :action=>"list",:search_condit=>params[:search_condit],:recevice_code=>params[:recevice_code],:order=>params[:order],:page=>params[:page]
   end
  #关于争议金额的处理
  def edit_dispute_amount
    ...
    if params[:recevice_code] == nil
      ...
      # @amount1 申请人争议金额; @amount2 被申请人争议金额
      ...
      if @case!=nil
        ...
        # 涉外案件收 仲裁费/立案费： 0003 涉外普通； 0004 涉外简易；
      ...
      #国内案件：收取受理费/处理费
        ...
        if respondent_all != 0
          @respondent_litigation_fee = Cost.new.get_01_01(respondent_all)  #受理费
          @respondent_processing_fee = Cost.new.get_01_02(respondent_all)  #处理费
        else
          @respondent_litigation_fee = 0
          @respondent_processing_fee = 0
        end
...
#    @isperson={1=>"是",2=>"否"}
  ...
  def case_copy_do
    ...
    num = params[:num].to_i
    while i<= num do
      recevice_code = SysArg.add_0001();
      cp.copy(@case.recevice_code, recevice_code, session[:user_code])
