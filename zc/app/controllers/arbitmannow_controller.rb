...
class ArbitmannowController < ApplicationController    
  ...
  def list_arbitmannow
    ...
    @tb_arbitmannow_pages,@tb_arbitmannows=paginate_by_sql(TbArbitmannow,"select tb_arbitmannows.arbitman_num as arbitman_num ,tb_arbitmen.name as name,tb_arbitmen.name_idcard as name_idcard ,tb_arbitmen.age as age,tb_arbitmen.spell as spell   from tb_arbitmen,tb_arbitmannows where #{c}  order by #{@order}",@page_lines.to_i)
  end
  ...
  def create_arbitmannow
    ...
    @tb_arbitman = TbArbitmannow.find_by_arbitman_num_and_used(params[:code],"Y")
    if @tb_arbitman == nil
      begin
  ...
  def create_arbitmannow_2
    ...
    begin
      array_con.each do |con|
        if TbArbitmannow.find_by_arbitman_num_and_used(con,"Y")==nil
          @tb_arbitman_expire_log=TbArbitmanExpireLog.new
          @tb_arbitman_expire_log = TbArbitmanExpireLog.new(params[:tb_arbitman_expire_logs])
  ...
  def create_dismiss
    begin
      ...
      @tb_arbitman_expire_log.arbitman_name_idcard = TbArbitman.find_by_code(params[:id]).name_idcard
      @tb_arbitman_expire_log.user = session[:user_code]
      @tb_arbitman_expire_log.dt = params[:tb_arbitman_expire_logs][:dt]
      @tb_arbitman_expire_log.reason = params[:tb_arbitman_expire_logs][:reason]
      @tb_arbitman_expire_log.expire = params[:tb_arbitman_expire_logs][:expire]
  ...
  def create_continuation_many
    ...
    begin
      array_con.each do |con|
        ...
        @tb_arbitman_expire_log.arbitman_num = con
        @tb_arbitman_expire_log.arbitman_name = TbArbitman.find_by_code(con).name
        @tb_arbitman_expire_log.arbitman_name_idcard = TbArbitman.find_by_code(con).name_idcard
        @tb_arbitman_expire_log.expire = params[:tb_arbitman_expire_logs][:expire]
        @tb_arbitman_expire_log.dt = params[:tb_arbitman_expire_logs][:dt]
        @tb_arbitman_expire_log.reason = params[:tb_arbitman_expire_logs][:reason]
        @tb_arbitman_expire_log.user = session[:user_code]
  ...
  def create_dismiss_many
    ...
    begin
      array_dis.each do |dis|
        ...
        @tb_arbitman_expire_log.arbitman_name_idcard = TbArbitman.find_by_code(dis).name_idcard
        @tb_arbitman_expire_log.expire = params[:tb_arbitman_expire_logs][:expire]
        @tb_arbitman_expire_log.dt = params[:tb_arbitman_expire_logs][:dt]
        @tb_arbitman_expire_log.reason = params[:tb_arbitman_expire_logs][:reason]
        @tb_arbitman_expire_log.user = session[:user_code]
