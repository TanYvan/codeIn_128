...
class AwardController < ApplicationController
  ...
  def award_create
    ...
    if @award.save
      @case=TbCase.find_by_recevice_code(session[:recevice_code])
      t_award=TbAward.find(:all,:conditions=>"used='Y' and recevice_code='#{@award.recevice_code}' and end_typ<>'0004'",:order=>"arbitman_submit_t asc")
      t_award_f=SysArg.get_first_record(t_award)
      if t_award_f==nil
        @case.award_id_first=nil
      else
        @case.award_id_first=t_award_f.id
      end

      t_award_l=SysArg.get_last_record(t_award)
      if t_award_l==nil
        @case.award_id_last=nil
      else
        @case.award_id_last=t_award_l.id
      end
      @case.save
