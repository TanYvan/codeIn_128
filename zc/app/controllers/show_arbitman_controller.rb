...
#创建时间 20090522
    ...
    def list_arbitman           
        ...
        @hant_search_1=[['char','tb_arbitmen_name','姓名','text',[]],
          ...
          ['char','tb_arbitmen_area_code','地区','selecttext',Region.find(:all).collect{|p|[p.code,p.name]}],
#          ['char','name','年龄','text',[]],    
          ['char','tb_arbitmen_salutation','称呼','text',[]],
          ['char','tb_arbitmen_zy','职业分类','select',TbDictionary.find(:all,:conditions=>"typ_code='9002' and state='Y' and used='Y'",:order=>'data_val',:select=>"data_val,data_text").collect{|p|[p.data_val,p.data_text]}],
          ['char','tb_arbitmen_company','单位','text',[]],
          ['char','tb_arbitmen_country','国籍','text',[]],
          ['char','tb_arbitmen_city','居住地','text',[]],
          ['char','tb_languages_language','外语语种','select',TbDictionary.find(:all,:conditions=>"typ_code='9008' and state='Y' and used='Y'",:order=>'data_val',:select=>"data_val,data_text").collect{|p|[p.data_val,p.data_text]}],
          ['char','specials_speciality_num','精通专业','select',TbDictionary.find(:all,:conditions=>"typ_code='9012' and state='Y' and used='Y'",:order=>'data_val',:select=>"data_val,data_text").collect{|p|[p.data_val,p.data_text]}],
...
#          ['char','name','一年内实际承接案件数','text',[]]
      ...
      'tb_arbitmen_code','tb_arbitmen_area_code','tb_arbitmen_salutation','tb_arbitmen_company',
    ...
    tb_arbitmen_name,tb_arbitmen_name_idcard,tb_arbitmen_sex,tb_arbitmen_area_code,tb_arbitmen_addr,tb_arbitmen_company,tb_arbitmen_mobiletel,tb_arbitmen_postcode
