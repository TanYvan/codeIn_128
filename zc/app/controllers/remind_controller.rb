...
class RemindController < ApplicationController
  ...
  def list
    ...
    @remind_pages,@remind=paginate(:reminds,:conditions=>"used='Y' and (state=1 or state=2) and uu='#{@user_code}' and dt1<='#{Date.today.to_s(:db)}'",:order=>@order,:per_page=>@page_lines.to_i)
  end
  #未到期提醒
  def will_list
    @order=params[:order]
  ...
  def will_update
    ...
    @remind1=Remind1.new
    @remind1.remind_id = params[:id]
            
    @remind1.dt1 = @remind.dt1 #原来日期
    @remind1.contents = @remind.contents #提醒内容
    if @remind.update_attributes(params[:remind])
      @remind1.dt2 = @remind.dt1
      @remind1.day1 = (@remind1.dt2.to_date - @remind1.dt1.to_date).to_i
      @remind1.state = 2
      ...
      flash[:notice]="修改成功"
      redirect_to :action=>"will_list"
    else
      flash[:notice]="修改失败"
      render :action=>"will_edit"
    end
  end
  #历史提醒
  def histroy_list
    @hant_search_1_page_name="histroy_list"
    @hant_search_1=[['char','contents','内容','text',[]],['date','dt1','提醒日期','text',[]]]
    @order=params[:order]
    if @order==nil or @order==""
      @order="dt1 desc,u_t desc"
    end
    ...
    c="used='Y' and state=3 and uu='#{session[:user_code]}' "
    p=PubTool.new()
    if @search_condit!= " and " and p.sql_check_3(@search_condit)!=false
      c = c + @search_condit
    end
    @remind_pages,@remind=paginate(:reminds,:conditions=>c,:order=>@order,:per_page=>@page_lines.to_i)
  end
  #创建自定义提醒
  def remind_new
    @remind=Remind.new()
  ...
  def complete
    ...
    begin
      ...
      flash[:notice] = "标记失败"
      render :action => "list",:page=>params[:page],:page_lines=>params[:page_lines],:user_code=>params[:user_code]
    else
      flash[:notice] = "标记成功"
      redirect_to :action => "list",:page=>params[:page],:page_lines=>params[:page_lines],:user_code=>params[:user_code]
    end
  end
  #确定延迟
  def delay_create
  ...
  def list_3
    ...
    @remind_pages,@remind=paginate(:reminds,:conditions=>"((reason_id like '%tb_charge_corrs-%1') or (reason_id like 'case_nowdate_t%')) and used='Y' and (state=1 or state=2) and uu='#{session[:user_code]}' and dt1<='#{Date.today.to_s(:db)}'",:order=>@order,:per_page=>@page_lines.to_i)
  end
  #收款提醒确定完成
  def complete_3
    @condi_k = params[:condi_k]
