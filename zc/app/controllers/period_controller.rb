...
class PeriodController < ApplicationController
  ...
  def period_list
    ...
    if @order==nil or @order==""
      @order="tb_expertconsults.name asc"
    end
  ...
  #批量增聘专家，同时在记录表和目前在聘专家表中添加记录
  def period_create_2
    @condi = params[:condi]
    array_con = @condi.split(",")
    begin
      array_con.each do |con|
        if TbPeriod.find_by_code(con)==nil
          @period_log = TbPeriodLog.new(params[:tb_period_logs])
          @period_log.code=con
          @period_log.expert_name=TbExpertconsult.find_by_code(con).name
          @period_log.user = session[:user_code]
          @period_log.action_type = '0003'
          @period_log.u_t = Time.now
          @period_log.save
      ...
      flash[:notice]="批量增聘专家失败！"
      render :action=>"expert_select",:page=>params[:page],:page_lines=>params[:page_lines]
    else
      flash[:notice]="批量增聘专家成功！"  
  ...
  def dismiss
    ...
    @tb_period_logs.dt=Date.today
    @expertconsult = TbExpertconsult.find_by_code(params[:id])
    @period = TbPeriodLog.find_by_code_and_used(params[:id],'Y').period
  end
  #确定解聘的操作
  def dismiss_expert
    begin
      ...
      @tb_period_log.period = params[:period]
      @tb_period_log.code = params[:id]
      @tb_period_log.expert_name = TbExpertconsult.find_by_code(params[:id]).name
      @tb_period_log.user = session[:user_code]
      @tb_period_log.u_t = Time.now
