...
class ApplicationController < ActionController::Base
  ...
  def user_role
    if self.controller_name == "screen_publish"
    elsif self.controller_name == "step1"
    elsif  self.controller_name == "case_consultation3" || self.controller_name == 'region_select'
      ...
      if session[:user_code] == nil
        ...
        puts "controller:"+self.controller_name
        puts "action:"+self.action_name
        ...
        if @act_1.empty? and @act_2.empty? and @act_3.empty? and @act_4.empty?
          render :text => "您无权访问该页面，请重新<a href='/welcome/login' target='_parent'>登录</a>"
        end
      end
    end
  end
