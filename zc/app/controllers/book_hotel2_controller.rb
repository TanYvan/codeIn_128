  ...
  def hotel
    ...
    if @year=="" or @year==nil or @workday1==nil
      ...
      for d in @workdays_months
        ...
        a=''
        @bookroom= TbArbithotel.find(:all,:select=>"a.usestyle as usestyle,a.arbitman as arbitman,a.case_code as case_code,a.rooms as rooms,a.days as days,a.id as id,s.add_typ as add_typ,a.state as state,s.area_code as area_code",:conditions=>"a.usedate='#{d.date.to_s(:db)}' and a.used='Y'",:joins=>"as a left join tb_sittings as s on a.case_code=s.case_code and a.sitting_id=s.id and s.used='Y'",:order=>"s.add_typ desc")
        for s in @bookroom
          ...
          uuu = ""
          clerk1 = TbCase.find_by_case_code(s.case_code)
          if clerk1!=nil
            clerk = clerk1.clerk
            uuu = User.find_by_code(clerk).name
          end
          sitt_place = ""
          if s.add_typ=='0002'
            sitt_place = "外地开庭:#{Region.find_by_code(s.area_code).name}<br/>"
          end
          if s.state=='0002'
            stat="待定"
          elsif s.state=='0003'
          ...
          a=a + sitt_place + "助理:#{uuu}<br>案件编号:#{s.case_code}<br>仲裁员:#{arb}(#{stat})<br>使用类型:#{q}<br>房间数量:#{s.rooms}间<br>住宿天数:#{s.days}天<br>"
          if @usestyle!='0001'
