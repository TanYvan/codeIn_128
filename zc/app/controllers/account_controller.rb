...
class AccountController < ApplicationController
  def list_account
    ...
    @hant_search_1=[['char','spell','姓名拼音缩写','text',[]],['char','name','姓名','text',[]],['char','bankremark','备注','text',[]]]
    @order=params[:order]
    ...
    #@account_pages,@account =paginate_by_sql(TbArbitmannow,"select a.name as name,a.id as id,a.spell as spell,a.sex as sex,a.mobiletel as mobiletel,a.bankaccount as bankaccount,a.bank_typ as bank_typ,a.bankname as bankname,a.id_card as id_card,a.addr as addr,a.bankremark as bankremark,a.bank_code as bank_code,a.email as email from tb_arbitmen as a,tb_arbitmannows as c where #{c} order by  #{@order}",@page_lines.to_i)
    @account_pages,@account =paginate_by_sql(TbArbitman,"select a.* from tb_arbitmen as a where #{c} order by  #{@order}",@page_lines.to_i)
  end
  #工作人员帐号信息
   def list_account_2
    @bank_typ={"0001"=>"本地","0002"=>"外地"}
