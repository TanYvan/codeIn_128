class ExtendController < ApplicationController
    ...
    @provide_p.each{|pp|
      if pp.typ == "1"
        ...
        if (@tax_a && @tax_b && @tax_a.rmb ==@tax_b.rmb) || !@tax_b
          ynssde = SysArg.round2f(tax.get_tax_base_2(pp.yf_rmb))
  ...
  def list_1
    ...
    c="extend_code='' and p not in (select code from tb_arbitmen where isunit='N')"
    p = PubTool.new()
    if @search_condit != " and " and p.sql_check_3(@search_condit) != false
      c = c + @search_condit
    end
            ...
            if typ == '1'
              ...
              if p_typ == '0001' or p_typ == '0002'
                ...
                if reward.used == 'Y' and reward.extend_code = ''
                  ...
                  ex.tax_base_rmb = tax.get_tax_base_2(ex.should_rmb.round)
  ...
  def search
    ...
    @tax_detail_marks = "''"
    @tb_extends.each{|e|
      @tax_detail_marks = @tax_detail_marks  + ",'#{e.tax_detail_mark}'"
    }
            ...
            if typ == '1'
              ...
              if p_typ == '0001' or p_typ == '0002' or p_typ=='0003'
                ...
                if reward.used == 'Y' and reward.extend_code = '' and reward.isextend = ''
                  ...
                  ex.should_rmb=(reward.zc_rmb * (1 + reward.bonus/100 - reward.penalty/100) + reward.gc_rmb).round
                  ex.tax_base_rmb = 0# tax.get_tax_base_1(ex.should_rmb.round)
                  ex.tax_rmb = 0 #ttt.rmb #tax.get_tax_1(ex.should_rmb.to_i,'N') # 本单位仲裁员不交税
                  ex.extend_rmb=ex.should_rmb - ex.tax_rmb
                  ex.extend_code=extend_code
                  ex.extend_u=session[:user_code]
                  ex.extend_t=t
  ...
  def arbitman_sum
    @date1 = params[:date1]
    @date2 = params[:date2]
    @arbitman_name = params[:arbitman_name]
    @case_code = params[:case_code]
    @date1 = Time.now.to_date.to_s(:db) if @date1.blank?
    @date2 = @date1 if @date2.blank?
    @bank_typ = {}
    TbDictionary.find(:all,:conditions=>"typ_code='8201'").each{|bbb|  @bank_typ.merge!({bbb.data_val => bbb.data_text})}
    @typ1 = {"0001" => "仲裁员", "0002" => "助理", "0003" => "校核人员", "0004" => "员工"}
    @typ2 = {"1" => "报酬", "2" => "奖励", "3" => "扣减", "4" => "办案其它报酬", "5" => "出差补助合计"}
    ...
    @order=params[:order]
    @order="p_name asc" if @order.blank?
    ...
    @search_condit = @search_condit + " and extend_date>='" + @date1 + "'"  unless @date1.blank?
    @search_condit = @search_condit + " and extend_date<='" + @date2 + "'"   unless @date2.blank?
    @search_condit = @search_condit + " and p_name='" + @arbitman_name + "'"   unless @arbitman_name.blank?
    @search_condit = @search_condit + " and case_code like '%" + @case_code + "%'"  unless @case_code.blank?
    
    @c=" used='Y' and typ<>'5' and p_typ='0001' and t_extend_code <>''  "
    ...
    @tb_extends_p = TbExtend.find(:all,:conditions=>@c,:select => "distinct p as p",:order=>@order)
  end



  # 已发放报酬查询
  def gxh_arbitman_sum
    @date1 = params[:date1]
    @date2 = params[:date2]
    @p = params[:p]
    @case_code = params[:case_code]
    ...
    @c=" used='Y' and typ<>'5' and p_typ='0001' and t_extend_code <>'' "
    @c = @c + " and extend_date>='" + @date1 + "'"  unless @date1.blank?
    @c = @c + " and extend_date<='" + @date2 + "'"   unless @date2.blank?
    @c = @c + " and p_name='" + @arbitman_name + "'"   unless @arbitman_name.blank?
    @c = @c + " and case_code like '%" + @case_code + "%'"  unless @case_code.blank?
    @c = @c + " and p=#{@p}" unless @p.blank?
    p = PubTool.new()
    if @search_condit != " and " and p.sql_check_3(@search_condit) != false
      @c = @c + @search_condit
    end
    @person = TbExtend.find(:all, :conditions => @c,:select =>"distinct p")
    #@extend_pages,@tb_extends = paginate(:tb_extends,:conditions=>c,:order=>@order,:per_page=>@page_lines.to_i)
  end
