class CaseQuery1Controller < ApplicationController   
  ...
  def case_now_clerk2_total
    ...
    if @search_condit!= " and " and p.sql_check_3(@order)!=false
      ...
      @case2=TbCase.find_by_sql("select users.name as clerk_name, (select count(1) from tb_cases  where tb_cases.clerk=users.code  and  tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.state<100 and tb_cases.end_code='' and tb_cases.aribitprog_num in ('0001','0003','0005','0007') and  tb_cases.clerk=users.code) as n1 ,  (select count(1) from tb_cases  where tb_cases.clerk=users.code  and  tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.state<100 and tb_cases.end_code='' and tb_cases.aribitprog_num in ('0002','0004','0006','0008') and  tb_cases.clerk=users.code) as n2 from users where users.code in (select distinct tb_cases.clerk from tb_cases,users where tb_cases.clerk=users.code and  tb_cases.used='Y' and tb_cases.state>=4 and tb_cases.state<100 and tb_cases.end_code='')  order by #{@order}")
      #user_duties.duty_code='0003' and users.code = user_duties.user_code
    end
  end
  ...
  def list_32
    ...
    @hant_search_1=[
      ...
      ['char','tb_cases_case_code','立案号','text',[]],['char','tb_cases_end_code','结案号','text',[]],
      ['char','tb_cases_clerk','办案助理','select',UserDuty.find_by_sql("select users.code as code,users.name as name from users,user_duties where  users.states='Y' and users.used='Y' and users.code=user_duties.user_code and user_duties.duty_code='0003' order by users.ord").collect{|p|[p.code,p.name]}.insert(0,["","全部"])],
      ['char','IFNULL(aname,\\\'\\\')','仲裁员姓名','text',[]],['char','IFNULL(party_partyname,\\\'\\\')','当事人','text',[]],
      ['char','IFNULL(party1_partyname,\\\'\\\')','申请人','text',[]],
      ['char','IFNULL(agent1_name,\\\'\\\')','申请人代理人','text',[]],['char','IFNULL(witne1_name,\\\'\\\')','申请人证人','text',[]],
      ['char','IFNULL(party2_partyname,\\\'\\\')','被申请人','text',[]],['char','IFNULL(agent2_name,\\\'\\\')','被申请人代理人','text',[]],
      ['char','IFNULL(witne2_name,\\\'\\\')','被申请人证人','text',[]],
      ['char','tb_cases_case_type1','主体类型','select',TbDictionary.find(:all,:conditions=>"typ_code='8140' and state='Y' and used='Y'",:order=>'data_val',:select=>"data_val,data_text").collect{|p|[p.data_val,p.data_text]}],
    ...
    @hant_search_1_list=['tb_cases_general_code','tb_cases_case_code','tb_cases_end_code','tb_cases_clerk',
      'IFNULL(aname,\\\'\\\')', 'IFNULL(party_partyname,\\\'\\\')',
