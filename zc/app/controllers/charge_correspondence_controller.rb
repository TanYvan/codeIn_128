class ChargeCorrespondenceController < ApplicationController
  ...
  def charge_list 
    ...
    c="used='Y' and state=1 and typ='a' and state_2=1"
  ...
  def case_list 
    ...
    if session[:charge_correspondence_charge_id]!=nil
      @charge=TbCharge.find(session[:charge_correspondence_charge_id])
    end
    ...
    c="used='Y' and ((clerk='#{session[:user_code]}' and state>=4 and state<=100) or ((clerk='#{session[:user_code]}' or clerk_2='#{session[:user_code]}') and state=1)) "
    p=PubTool.new()
  ...
  def correspondence_do
    ...
    if TbCase.find(case_id).clerk==session[:user_code] or TbCase.find(case_id).clerk==''
      ...
      render :text=>"<link href='/stylesheets/style.css' rel='stylesheet' type='text/css'> 对不起，您不是本案办案秘书，无法完成费用对应操作。      <br>    <a href='/charge_correspondence/case_list?charge_id=#{params[:charge_id]}'>返回到 收款信息</a>"
    end
  end
  ...
  def update
    ...
    if @charge.update_attributes(params[:charge])
      flash[:notice]="修改成功"
      redirect_to :action=>"charge_list"
    else
      flash[:notice]="修改失败"
      render :action=>"edit"
    end
