...
class RehearingController < ApplicationController
  def list
    ...
    c="used='Y' and state>=4 and state<=150 and clerk='#{session[:user_code]}' and caseendbook_id_first is not null"
    p=PubTool.new()
  ...
  def final
    ...
    @case=TbCase.find_by_sql("insert into tb_cases (used,state,recevice_code,general_code,
     ...
     nowdate,limit_dat,finally_limit_dat,re_code,app_rules)
     ...
     orgstyle,accepttime,aribittype,terms_drafting_party,special,nowdate,limit_dat,finally_limit_dat,recevice_code,app_rules
