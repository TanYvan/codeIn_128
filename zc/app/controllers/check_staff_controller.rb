class CheckStaffController < ApplicationController
  ...
  def new
    ...
    @check_staff.check_time = Date.today
  ...
  def update
     ...
     @check_staff.u=session[:user_code]
     @check_staff.u_t=Time.now()
     if @check_staff.update_attributes(params[:check_staff])
        flash[:notice]="修改成功"
        redirect_to :action=>"list",:recevice_code=>params[:recevice_code],:search_condit=>params[:search_condit],:order=>params[:order],:page_lines=>params[:page_lines]
     else
        flash[:notice]="修改失败"
        render :action=>"edit",:id=>params[:id],:recevice_code=>params[:recevice_code],:search_condit=>params[:search_condit],:order=>params[:order],:page_lines=>params[:page_lines]
     end
