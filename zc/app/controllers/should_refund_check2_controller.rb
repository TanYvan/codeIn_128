class ShouldRefundCheck2Controller < ApplicationController
  def list
    ...
    @should=TbShouldRefund.find(:all,:order=>'right(tb_cases.case_code,7) desc,tb_should_refunds.recevice_code desc',:joins=>" inner join tb_cases  on tb_should_refunds.recevice_code=tb_cases.recevice_code and tb_cases.used='Y' and tb_should_refunds.used='Y' and tb_should_refunds.a_state<>1",:select=>s,:conditions=>c)
  end  
