class FilingRacallController < ApplicationController
  ...
  def recall_do
    ...
    if @case.state==200
       f=TbCaseFlow.del_flow(@case.recevice_code,'0009')
       if f!=0
         @case.state=f
       end 
       @case.file_receive_u=''
       ...
       if @case.update_attributes(params[:case]) 
          flash[:notice]="立案号：#{@case.case_code}案件接收回退成功"
       else
          flash[:notice]="立案号：#{@case.case_code}案件接收回退失败"
       end
