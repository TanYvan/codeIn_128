class ChargeCorrespondenceDetailController < ApplicationController
  ...
  def charge_case_list
    ...
    if @order==nil or @order==""
      @order="v_case_query1_list1s.tb_cases_general_code desc"
    end
    ...
    c="tb_charges.used='Y' and v_case_query1_list1s.tb_cases_recevice_code=tb_charges.recevice_code and ((v_case_query1_list1s.tb_cases_clerk='#{session[:user_code]}' and v_case_query1_list1s.tb_cases_state>=4 and v_case_query1_list1s.tb_cases_state<=100) or ((v_case_query1_list1s.tb_cases_clerk='#{session[:user_code]}' or v_case_query1_list1s.tb_cases_clerk_2='#{session[:user_code]}') and v_case_query1_list1s.tb_cases_state>=1 and v_case_query1_list1s.tb_cases_state<3)) "
    p=PubTool.new()
  ...
  def charge_corr_create
    ...
    if @sss.typ == "0009" #如果类型为 “多收仲裁费”，需要给此项应收费赋值，分配给多少就赋值多少
      @sss.rmb_money = @ccc.rmb_money + @sss.rmb_money
      @sss.f_money   = @sss.rmb_money
      @sss.u         = "System"
      @sss.u_t       = Time.now()
      @sss.save
    end

    #if (@sss.rmb_money - @sss.redu_rmb_money - @sss.re_rmb_money  < @ccc.rmb_money - 300)  and params[:che]==nil
    ...
    if (@sss.rmb_money - @sss.redu_rmb_money - @sss.re_rmb_money  < @ccc.rmb_money)
      flash[:notice]="添加失败！原因：对应金额(#{@ccc.rmb_money})不能大于欠交金额(#{@sss.rmb_money - @sss.redu_rmb_money - @sss.re_rmb_money})。"
      redirect_to :action=>"charge_corr_list",:should_charge_id=>params[:should_charge_id],:recevice_code=>params[:recevice_code]
    else
      ...
      if (TbCase.find_by_recevice_code(params[:recevice_code]).clerk==session[:user_code] or TbCase.find_by_recevice_code(params[:recevice_code]).clerk=='') and  @charge.recevice_code==params[:recevice_code]
        ...
        flash[:notice]="添加失败,请用本案办案助理身份登录进行此项操作！"
        redirect_to :action=>"charge_corr_list",:should_charge_id=>params[:should_charge_id],:recevice_code=>params[:recevice_code]
      end
  ...
  def charge_corr_delete
    ...
    @sss=TbShouldCharge.find(@charge_corr.should_charge_id)
    @ccc=TbCharge.find(@charge_corr.charge_id)

    if @sss.typ == "0009" #如果类型为 “多收仲裁费”，需要给此项应收费赋值，分配给多少就赋值多少
      @sss.rmb_money = @sss.rmb_money - @ccc.rmb_money
      @sss.f_money   = @sss.rmb_money
      @sss.u         = "System"
      @sss.u_t       = Time.now()
      @sss.save
  ...
  def split_new
    ...
    @charge.f_money=SysArg.round_2(@charge.f_money - @f_money)
    @charge.rmb_money=SysArg.round_2(@charge.rmb_money - @rmb_money)
    @charge.remark=""
  end
  ...
  def charge_update
    ...
    if @charge.update_attributes(params[:charge])
      TbCharge.update_all("ticket_typ='#{@charge.ticket_typ}'","parent_id=#{@charge.id}")
      flash[:notice]="修改成功"
      redirect_to :action=>"list",:recevice_code=>params[:recevice_code]
    else
      flash[:notice]="修改失败"
      render :action=>"charge_edit",:recevice_code=>params[:recevice_code]
    end
  end
end
