class ArbitmanSpendController < ApplicationController
  ...
  def create
    ...
    if @c.state>=4
      ...
      @arb=TbCasearbitman.find(:all,:conditions=>"used='Y' and recevice_code='#{@arbitman_spend.recevice_code}' and arbitman='#{@arbitman_spend.arbitman}'")
  ...
  def delete
    ...
    if @c.state>=4
      ...
      @arbitman_spend.u_t=Time.now()
      if @arbitman_spend.save
        #ArbitmanSpend.update_all("used='N'","parent_id=#{@arbitman_spend.id}")
        flash[:notice]="删除成功"
      else
        flash[:notice]="删除失败"
      end
