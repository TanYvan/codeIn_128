class CaseEndController < ApplicationController
  ...
  def list
    ...
    r17 = "" #有仲裁员的案件必须处长把报酬信息提交到财务
    r18 = "" #仲裁员提交裁决时间稿时间是否填写
    @case = nil#当前办理案件
    if session[:recevice_code]!=nil
      ...
      if @caseendbook==nil#没有结案信息
        ...
        if @endStyle!='0003'
          ...
          r15 += "没有录入工时汇总内的裁决书制作信息；" if @remun1.length == 0
          r15 += "没有录入绩效评价信息；" if (@caseendbook.decideDate.to_s(:db) < "2013-03-01") && @remun2.length == 0
...
#          end
      ...
      r17 = "存在仲裁员无报酬记录；" if TbCasearbitman.count("recevice_code='#{session[:recevice_code]}' and used='Y'")>0 && @case.remun_state=="N"

      r18=""
      if TbAward.find(:first, :conditions=> "recevice_code='#{session[:recevice_code]}' and used='Y' and end_typ<>'0004' and arbitman_submit_t is null")
        r18="仲裁员提交裁决稿时间未填写"
      elsif TbAward.find(:first, :conditions=> "recevice_code='#{session[:recevice_code]}' and used='Y' and end_typ<>'0004' and award_submit_t is null")
        r18="裁决书报校核时间未填写"
      elsif TbAward.find(:first, :conditions=> "recevice_code='#{session[:recevice_code]}' and used='Y' and end_typ<>'0004' and award_return_t is null")
        r18="裁决书报校核返回时间未填写"
      end
      
      @r_mes = r0 + r14 + r3 + r10 + r1 + r2 + r4 + r5 + r6 + r7 + r8 + r9 + r11 + r12 + r13 + r15 + r16 + r17 + r18
      ...
      if TbShouldCharge.sum(:rmb_money,:conditions=>["used='Y' and recevice_code=? and typ='0009'",session[:recevice_code]]) != 0
        s4="请注意，有当事人多交仲裁费；"
      else
        s4=""
      end
