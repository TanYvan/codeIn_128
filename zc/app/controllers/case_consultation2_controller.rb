...
class CaseConsultation2Controller < ApplicationController
...
#      @case=TbCase.find(:all,:conditions=>c,:order=>@order)
      @case == nil
    elsif @typ == '1'
      @case = TbCase.find_by_sql("select * from tb_cases where u='#{session[:user_code]}' and used='Y' and state=1
                  and recevice_code_limit_dat>='#{Date.today}'  order by id desc limit 1")
    elsif @typ == '2'
      @case == nil
    end
  end
  ...
  def edit
    ...
    #案件应收费用信息（用于判断是否可以立案，只有应收费用 rmb_money 不为0 才可以立案）
  ...
  def clerk_select
    ...
    #指定或者更换办案助理的时候需要更新助理变更信息表 tb_clerkchanges
